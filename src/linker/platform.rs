...
impl PlatformConfig {
    ...
    pub(super) fn new() -> Result<Self> {
        if env::consts::ARCH == "x86_64" && env::consts::OS == "linux" {
            ...
            miette::bail!("Unsupported platform/architecture ({}, {}) for linker.", env::consts::OS, env::consts::ARCH);
        }
    }
    ...
    fn detect_linux_x64() -> Result<Self> {
        ...
        if !crt.exists() {
            miette::bail!("linker: Could not find platform `crt1.o` -- file not found @ {}.", crt.to_str().unwrap());
        }
        if !loader.exists() {
            miette::bail!("linker: Could not find platform `ld64.so` -- file not found @ {}.", loader.to_str().unwrap());
        }
