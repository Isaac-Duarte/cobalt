...
pub(super) fn upload_literals(module: &mut ObjectModule, literals: &BiMap<LiteralId, String>) -> Result<HashMap<LiteralId, DataId>> {
    ...
    for (lit_id, literal) in literals.iter() {
        ...
        let data_id = module.declare_anonymous_data(false, false).expect("Failed to declare literal data.");
        desc.clear();
        let literal_bytes = literal.clone().into_bytes().into_boxed_slice();
        desc.define(literal_bytes);
        module.define_data(data_id, &desc).expect("Failed to define literal data bytes.");
